...
add_device_id() {
    ...
    if [[ -z "$device_id" ]]; then
        ...
        echo "Starting PingPong app in the 'pingpong_main' screen session with your device ID..."
...
cleanup() {
    ...
    # 删除所有相关的 screen 会话
    echo "Deleting all 'pingpong_main' screen sessions..."
    screen -ls | grep 'pingpong_main' | awk '{print $1}' | while read -r session; do
...
show_menu() {
    ...
    echo "4. 添加设备ID并在 'pingpong_main' 会话中运行 PingPong 应用"
    echo "5. 查看日志 (Ctrl + A + D 退出)"
    echo "6. 配置 AIOZ 账户"
    echo "7. 删除所有相关的文件和会话（不包括 Docker 和 screen）"
    echo "8. 退出"
}
